...
<body>
    ...
    if ($_SERVER['REQUEST_METHOD'] == "POST") {
        ...
        if (empty($errors)) {
            $cap_nhat_hoc_vien = mysqli_prepare($conn, "UPDATE hoc_vien SET ho_lot_hoc_vien=?, ten_hoc_vien=?, so_dien_thoai=?, email=?, can_cuoc_cong_dan=?, ngay_sinh=?");
            ...
            if (mysqli_stmt_execute($cap_nhat_hoc_vien)) {
                $cap_nhat_dang_ky = mysqli_prepare($conn, "UPDATE dang_ky SET so_tien_da_dong=?, khoa_hoc_id=?");
                mysqli_stmt_bind_param($cap_nhat_dang_ky, 'is', $so_tien_da_dong, $khoa_hoc_id);
                if (mysqli_stmt_execute($cap_nhat_dang_ky)) {
                    header("Location: list_register.php");
                    exit();
                } else {
                    $errors[] = "Lỗi cập nhật đăng ký " . mysqli_error($conn);
                }
            } else {
                $errors[] = "Lỗi cập nhật học viên " . mysqli_error($conn);
            }
        }
    }
    ?>
    ...
    <form action="" method="POST">
        ...
        Ngày sinh: <input type="text" name="ngay_sinh" value="<?= $row['ngay_sinh'] ?>"> <br>
        Khóa học:
